...
// --- EXERCISE 1: Variable Support (Inheritance) ---
// TODO: Implement VariableNode. 
// It should represent the variable 'x'. 
// When evaluated, it should return the 'x' value stored in EvalContext.
struct VariableNode : ASTNode {
    // Hint: Look at how NumberNode is implemented. 
    // What should evaluate() return if it needs to represent 'x'?

};

// --- EXERCISE 2: Unary Operators (Ownership) ---
// TODO: Implement UnaryOpNode to support unary minus (e.g., -x or -(5+3)).
struct UnaryOpNode : ASTNode {
    char op;
    std::unique_ptr<ASTNode> child;

    // Hint: How do you handle ownership of 'child' in the constructor? 
    // Remember unique_ptr cannot be copied.
    UnaryOpNode(char o, std::unique_ptr<ASTNode> c) : op(o), child(nullptr) {
        /* Your code here: move 'c' into 'child' */
    }

    double evaluate(const EvalContext& ctx) const override {
        double val = child->evaluate(ctx);
        if (op == '-') return -val;
        return val;
    }

    // Hint: For clone(), you must recursively clone the child.
    std::unique_ptr<ASTNode> clone() const override {
        return nullptr; // Replace this
    }

    std::string toString() const override {
        return "(" + std::string(1, op) + child->toString() + ")";
    }
};
