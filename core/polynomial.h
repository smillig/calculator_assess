...
class Polynomial
{
    std::vector<T> coeffs_; // coeffs_[i] is the coefficient for x^i
public:
    Polynomial() = default;
    ...
    /*
    Polynomial(std::initializer_list<T> c) : coeffs_(c) {}

    // --- EXERCISE 5: Scalar Multiplication (Templates/Overloading) ---
    // TODO: Overload the * operator so we can do "myPoly * 5.0".
    Polynomial<T> operator*(T scalar) const {
        // Hint: Create a new vector where every element is multiplied by scalar.
        // Return a new Polynomial built from that vector.
    }

    // --- EXERCISE 6: Horner's Method (Algorithm) ---
    // Efficiently evaluate the polynomial: a + bx + cx^2...
    T evaluate(T x) const {
        T result = 0;
        // TODO: Implement the loop for Horner's Method.
        // Hint: Start from the highest degree coefficient and work backwards.
