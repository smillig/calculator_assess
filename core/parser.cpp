...
namespace Parser
{
    ...
    // --- EXERCISE 3: The Lexer (Variable Recognition) ---
    // Inside the tokenize loop, we need to recognize 'x'.
    std::vector<Token> tokenize(const std::string& s, std::optional<std::string>& err)
    {
        std::vector<Token> out;
        size_t i = 0, n = s.size();
        /*
        Exercise 3 refactor to:
        for (size_t i = 0; i < s.size();) {
            if (std::isspace(s[i])) { i++; continue; }

            // TODO: If the character is 'x', add a TOK_VAR token.
            // Hint: Look at how TOK_OP is handled below.
            
            // ... (existing number and operator parsing) ...
        }
        return out;
    ...
    // --- EXERCISE 4: Unary Minus Logic (Shunting-Yard Invariants) ---
    // A '-' is unary if it's the first token or follows another operator/paren.
    // Example: "-5 + (-x)"
    ParseResult shuntingYardToAST(const std::vector<Token>& tokens)
    {
        ...
        /*
        exercise 4 insert the following to check for urinary operators:
        ...
        for (const auto& t : tokens) {
            if (t.type == TOK_NUMBER) {
                ...
                // TODO: Push a VariableNode to 'vals' and set expectUnary to false.
            }
            else if (t.type == TOK_OP) {
                if (expectUnary && t.op == '-') {
                    // TODO: How do we handle a Unary Operator on the stack?
                    // Hint: You might need a special marker or push it to 'ops' 
                    // with very high precedence.
                }
                // ... (standard binary op logic) ...
                expectUnary = true; 
            }
            // ... (paren logic) ...
        }
